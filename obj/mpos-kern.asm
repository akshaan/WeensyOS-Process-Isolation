...
schedule(void)
{
  ...
  10008f:	a1 70 9f 10 00       	mov    0x109f70,%eax
	while (1) {
		pid = (pid + 1) % NPROCS;
...
schedule(void)
{
  ...
  1000a4:	83 b8 10 92 10 00 01 	cmpl   $0x1,0x109210(%eax)
  1000ab:	75 ee                	jne    10009b <schedule+0xf>
			run(&proc_array[pid]);
  1000ad:	83 ec 0c             	sub    $0xc,%esp
  1000b0:	05 c8 91 10 00       	add    $0x1091c8,%eax
  1000b5:	50                   	push   %eax
...
interrupt(registers_t *reg)
{
  ...
  1000c4:	83 ec 1c             	sub    $0x1c,%esp
	...
	// registers into the 'current' process descriptor.
	current->p_registers = *reg;
  1000c7:	8b 1d 70 9f 10 00    	mov    0x109f70,%ebx
...
interrupt(registers_t *reg)
{
  ...
  1000fc:	b8 64 92 10 00       	mov    $0x109264,%eax
  ...
  100124:	8d 85 c8 91 10 00    	lea    0x1091c8(%ebp),%eax
  ...
  100131:	8b b5 c8 91 10 00    	mov    0x1091c8(%ebp),%esi
	}
  ...
  10015c:	e8 b3 03 00 00       	call   100514 <memcpy>
  ...
  100168:	8b 54 24 08          	mov    0x8(%esp),%edx
	...
	return -1;
 	...
 	proc_array[child_pid].p_registers = parent->p_registers; // set child regs	
        copy_stack(&proc_array[child_pid],parent);      // set child stack
        proc_array[child_pid].p_registers.reg_eax = 0;  // set exit code
  10016c:	c7 85 e8 91 10 00 00 	movl   $0x0,0x1091e8(%ebp)
  100173:	00 00 00 
	proc_array[child_pid].p_state = P_RUNNABLE;
  100176:	c7 85 10 92 10 00 01 	movl   $0x1,0x109210(%ebp)
  10017d:	00 00 00 
	dest_stack_bottom = dest_stack_top + (src_stack_bottom - src_stack_top);
  ...
  100189:	ff 35 70 9f 10 00    	pushl  0x109f70
  10018f:	e8 a8 02 00 00       	call   10043c <run>
	case INT_SYS_YIELD:
  ...
  100199:	a1 70 9f 10 00       	mov    0x109f70,%eax
  ...
  1001ad:	83 b8 64 92 10 00 02 	cmpl   $0x2,0x109264(%eax)
  1001b4:	75 0a                	jne    1001c0 <interrupt+0x105>
				proc_array[i].p_state = P_RUNNABLE;
  1001b6:	c7 80 64 92 10 00 01 	movl   $0x1,0x109264(%eax)
  1001bd:	00 00 00 
  1001c0:	83 c0 54             	add    $0x54,%eax
		...
		int i =0;
		for(i = 1; i <NPROCS; i++)
  1001c3:	3d ec 04 00 00       	cmp    $0x4ec,%eax
  ...
  1001cf:	a1 70 9f 10 00       	mov    0x109f70,%eax
  1001d4:	8b 50 20             	mov    0x20(%eax),%edx
		if (p <= 0 || p >= NPROCS || p == current->p_pid
  1001d7:	8d 4a ff             	lea    -0x1(%edx),%ecx
  1001da:	83 f9 0e             	cmp    $0xe,%ecx
  1001dd:	77 14                	ja     1001f3 <interrupt+0x138>
  1001df:	3b 10                	cmp    (%eax),%edx
  1001e1:	74 10                	je     1001f3 <interrupt+0x138>
		    || proc_array[p].p_state == P_EMPTY)
  1001e3:	6b d2 54             	imul   $0x54,%edx,%edx
  1001e6:	8d 8a d0 91 10 00    	lea    0x1091d0(%edx),%ecx
  1001ec:	8b 59 40             	mov    0x40(%ecx),%ebx
		// (In the Unix operating system, only process P's parent
		// can call sys_wait(P).  In MiniprocOS, we allow ANY
		// process to call sys_wait(P).)
		...
		pid_t p = current->p_registers.reg_eax;
		if (p <= 0 || p >= NPROCS || p == current->p_pid
  1001ef:	85 db                	test   %ebx,%ebx
  1001f1:	75 09                	jne    1001fc <interrupt+0x141>
		    || proc_array[p].p_state == P_EMPTY)
			current->p_registers.reg_eax = -1;
  1001f3:	c7 40 20 ff ff ff ff 	movl   $0xffffffff,0x20(%eax)
		// (In the Unix operating system, only process P's parent
		// can call sys_wait(P).  In MiniprocOS, we allow ANY
		// process to call sys_wait(P).)
		...
		pid_t p = current->p_registers.reg_eax;
		if (p <= 0 || p >= NPROCS || p == current->p_pid
  1001fa:	eb 1e                	jmp    10021a <interrupt+0x15f>
		    || proc_array[p].p_state == P_EMPTY)
			current->p_registers.reg_eax = -1;
		else if (proc_array[p].p_state == P_ZOMBIE){
  1001fc:	83 fb 03             	cmp    $0x3,%ebx
  1001ff:	75 12                	jne    100213 <interrupt+0x158>
			current->p_registers.reg_eax = proc_array[p].p_exit_status;			proc_array[p].p_state = P_EMPTY;}
  100201:	8b 92 14 92 10 00    	mov    0x109214(%edx),%edx
  100207:	c7 41 40 00 00 00 00 	movl   $0x0,0x40(%ecx)
  10020e:	89 50 20             	mov    %edx,0x20(%eax)
  100211:	eb 07                	jmp    10021a <interrupt+0x15f>
		else
			//current->p_registers.reg_eax = WAIT_TRYAGAIN;
			current->p_state = P_BLOCKED;
  100213:	c7 40 48 02 00 00 00 	movl   $0x2,0x48(%eax)
		schedule();
...
start(void)
{
  ...
  10022c:	68 c8 91 10 00       	push   $0x1091c8
  100231:	e8 42 03 00 00       	call   100578 <memset>
  100236:	ba c8 91 10 00       	mov    $0x1091c8,%edx
  10023b:	31 c0                	xor    %eax,%eax
  10023d:	83 c4 10             	add    $0x10,%esp
	for (i = 0; i < NPROCS; i++) {
		proc_array[i].p_pid = i;
  100240:	89 02                	mov    %eax,(%edx)
	int whichprocess;
	pid_t i;
  ...
  100243:	c7 42 48 00 00 00 00 	movl   $0x0,0x48(%edx)
	int whichprocess;
	pid_t i;
	...
	memset(proc_array, 0, sizeof(proc_array));
	for (i = 0; i < NPROCS; i++) {
  10024a:	83 c2 54             	add    $0x54,%edx
  10024d:	83 f8 10             	cmp    $0x10,%eax
  100250:	75 ee                	jne    100240 <start+0x1f>
		proc_array[i].p_pid = i;
		proc_array[i].p_state = P_EMPTY;
	}
  ...
  100252:	c7 05 70 9f 10 00 1c 	movl   $0x10921c,0x109f70
  100259:	92 10 00 
	...
	// first process.
	segments_init();
  10025c:	e8 73 00 00 00       	call   1002d4 <segments_init>
	special_registers_init(current);
  100261:	83 ec 0c             	sub    $0xc,%esp
  100264:	ff 35 70 9f 10 00    	pushl  0x109f70
  10026a:	e8 e4 01 00 00       	call   100453 <special_registers_init>

	// Erase the console, and initialize the cursor-position shared
	// variable to point to its upper left.
	console_clear();
  10026f:	e8 2f 01 00 00       	call   1003a3 <console_clear>

	// Figure out which program to run.
	cursorpos = console_printf(cursorpos, 0x0700, "Type '1' to run mpos-app, or '2' to run mpos-app2.");
  100274:	83 c4 0c             	add    $0xc,%esp
  100277:	68 08 0a 10 00       	push   $0x100a08
  10027c:	68 00 07 00 00       	push   $0x700
  100281:	ff 35 00 00 06 00    	pushl  0x60000
  100287:	e8 4e 07 00 00       	call   1009da <console_printf>
  10028c:	83 c4 10             	add    $0x10,%esp
  10028f:	a3 00 00 06 00       	mov    %eax,0x60000
	do {
		whichprocess = console_read_digit();
  100294:	e8 4d 01 00 00       	call   1003e6 <console_read_digit>
	} while (whichprocess != 1 && whichprocess != 2);
  100299:	8d 58 ff             	lea    -0x1(%eax),%ebx
  10029c:	83 fb 01             	cmp    $0x1,%ebx
  10029f:	77 f3                	ja     100294 <start+0x73>
	console_clear();
  1002a1:	e8 fd 00 00 00       	call   1003a3 <console_clear>
  ...
  1002a6:	50                   	push   %eax
  1002a7:	50                   	push   %eax
  1002a8:	a1 70 9f 10 00       	mov    0x109f70,%eax
  1002ad:	83 c0 34             	add    $0x34,%eax
  1002b0:	50                   	push   %eax
  1002b1:	53                   	push   %ebx
  1002b2:	e8 d1 01 00 00       	call   100488 <program_loader>

	// Set the main process's stack pointer, ESP.
	current->p_registers.reg_esp = PROC1_STACK_ADDR + PROC_STACK_SIZE;
  1002b7:	a1 70 9f 10 00       	mov    0x109f70,%eax
  1002bc:	c7 40 40 00 00 2c 00 	movl   $0x2c0000,0x40(%eax)

	// Mark the process as runnable!
	current->p_state = P_RUNNABLE;
  1002c3:	c7 40 48 01 00 00 00 	movl   $0x1,0x48(%eax)

	// Switch to the main process using run().
	run(current);
  1002ca:	89 04 24             	mov    %eax,(%esp)
  1002cd:	e8 6a 01 00 00       	call   10043c <run>
  1002d2:	90                   	nop
  1002d3:	90                   	nop
...
segments_init(void)
{
  ...
  1002d4:	b8 08 97 10 00       	mov    $0x109708,%eax
...
segments_init(void)
{
  ...
  1002f5:	88 15 a8 1a 10 00    	mov    %dl,0x101aa8
	...
	for (i = 0; i < sizeof(interrupt_descriptors) / sizeof(gatedescriptor_t); i++)
		SETGATE(interrupt_descriptors[i], 0,
  1002fb:	ba 70 97 10 00       	mov    $0x109770,%edx
segments_init(void)
{
	int i;
...
segments_init(void)
{
  ...
  10031e:	c7 05 0c 97 10 00 00 	movl   $0x80000,0x10970c
  100325:	00 08 00 
	kernel_task_descriptor.ts_ss0 = SEGSEL_KERN_DATA;
  100328:	66 c7 05 10 97 10 00 	movw   $0x10,0x109710
  10032f:	10 00 
	...
	for (i = 0; i < sizeof(interrupt_descriptors) / sizeof(gatedescriptor_t); i++)
		SETGATE(interrupt_descriptors[i], 0,
  100331:	66 89 0c c5 70 97 10 	mov    %cx,0x109770(,%eax,8)
  100338:	00 
...
run(process_t *proc)
{
  10043c:	8b 44 24 04          	mov    0x4(%esp),%eax
	current = proc;
  100440:	a3 70 9f 10 00       	mov    %eax,0x109f70
