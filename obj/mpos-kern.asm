...
multiboot_start:
  ...
  100013:	9d                   	popf   
	call start
  100014:	e8 fe 01 00 00       	call   100217 <start>
  100019:	90                   	nop
...
schedule(void)
{
  ...
  10008f:	a1 64 9f 10 00       	mov    0x109f64,%eax
	while (1) {
		pid = (pid + 1) % NPROCS;
...
schedule(void)
{
  ...
  1000a4:	83 b8 04 92 10 00 01 	cmpl   $0x1,0x109204(%eax)
  1000ab:	75 ee                	jne    10009b <schedule+0xf>
			run(&proc_array[pid]);
  1000ad:	83 ec 0c             	sub    $0xc,%esp
  1000b0:	05 bc 91 10 00       	add    $0x1091bc,%eax
  1000b5:	50                   	push   %eax
...
interrupt(registers_t *reg)
{
  ...
  1000c4:	83 ec 1c             	sub    $0x1c,%esp
	...
	// registers into the 'current' process descriptor.
	current->p_registers = *reg;
  1000c7:	8b 1d 64 9f 10 00    	mov    0x109f64,%ebx
...
interrupt(registers_t *reg)
{
  ...
  1000e1:	0f 87 2e 01 00 00    	ja     100215 <interrupt+0x15a>
  ...
  1000fc:	b8 58 92 10 00       	mov    $0x109258,%eax
  ...
  100124:	8d 85 bc 91 10 00    	lea    0x1091bc(%ebp),%eax
  ...
  100131:	8b b5 bc 91 10 00    	mov    0x1091bc(%ebp),%esi
	}
  ...
  10015c:	e8 a7 03 00 00       	call   100508 <memcpy>
  ...
  100168:	8b 54 24 08          	mov    0x8(%esp),%edx
	...
	return -1;
 	...
 	proc_array[child_pid].p_registers = parent->p_registers; // set child regs	
        copy_stack(&proc_array[child_pid],parent);      // set child stack
        proc_array[child_pid].p_registers.reg_eax = 0;  // set exit code
  10016c:	c7 85 dc 91 10 00 00 	movl   $0x0,0x1091dc(%ebp)
  100173:	00 00 00 
	proc_array[child_pid].p_state = P_RUNNABLE;
  100176:	c7 85 04 92 10 00 01 	movl   $0x1,0x109204(%ebp)
  10017d:	00 00 00 
	dest_stack_bottom = dest_stack_top + (src_stack_bottom - src_stack_top);
  ...
  100189:	ff 35 64 9f 10 00    	pushl  0x109f64
  10018f:	e8 9c 02 00 00       	call   100430 <run>
	case INT_SYS_YIELD:
  ...
  100199:	a1 64 9f 10 00       	mov    0x109f64,%eax
  ...
  1001ab:	31 c0                	xor    %eax,%eax
		int i =0;
		for(i = 0; i <= NPROCS; i++)
		{
			if(proc_array[i].p_state == P_BLOCKED)
  1001ad:	83 b8 04 92 10 00 02 	cmpl   $0x2,0x109204(%eax)
  1001b4:	75 0a                	jne    1001c0 <interrupt+0x105>
				proc_array[i].p_state = P_RUNNABLE;
  1001b6:	c7 80 04 92 10 00 01 	movl   $0x1,0x109204(%eax)
  1001bd:	00 00 00 
  1001c0:	83 c0 54             	add    $0x54,%eax
		// changed by now, but we can read the APPLICATION's setting
		// for this register out of 'current->p_registers'.
		current->p_state = P_ZOMBIE;
		current->p_exit_status = current->p_registers.reg_eax;
		int i =0;
		for(i = 0; i <= NPROCS; i++)
  1001c3:	3d 94 05 00 00       	cmp    $0x594,%eax
  1001c8:	75 e3                	jne    1001ad <interrupt+0xf2>
		{
			if(proc_array[i].p_state == P_BLOCKED)
				proc_array[i].p_state = P_RUNNABLE;
		}
		schedule();
  1001ca:	e8 bd fe ff ff       	call   10008c <schedule>
  ...
  1001cf:	a1 64 9f 10 00       	mov    0x109f64,%eax
  1001d4:	8b 50 20             	mov    0x20(%eax),%edx
		if (p <= 0 || p >= NPROCS || p == current->p_pid
  1001d7:	8d 4a ff             	lea    -0x1(%edx),%ecx
  1001da:	83 f9 0e             	cmp    $0xe,%ecx
  1001dd:	77 11                	ja     1001f0 <interrupt+0x135>
  1001df:	3b 10                	cmp    (%eax),%edx
  1001e1:	74 0d                	je     1001f0 <interrupt+0x135>
		    || proc_array[p].p_state == P_EMPTY)
  1001e3:	6b d2 54             	imul   $0x54,%edx,%edx
  1001e6:	8b 8a 04 92 10 00    	mov    0x109204(%edx),%ecx
		// (In the Unix operating system, only process P's parent
		// can call sys_wait(P).  In MiniprocOS, we allow ANY
		// process to call sys_wait(P).)
		...
		pid_t p = current->p_registers.reg_eax;
		if (p <= 0 || p >= NPROCS || p == current->p_pid
  1001ec:	85 c9                	test   %ecx,%ecx
  1001ee:	75 09                	jne    1001f9 <interrupt+0x13e>
		    || proc_array[p].p_state == P_EMPTY)
			current->p_registers.reg_eax = -1;
  1001f0:	c7 40 20 ff ff ff ff 	movl   $0xffffffff,0x20(%eax)
		// (In the Unix operating system, only process P's parent
		// can call sys_wait(P).  In MiniprocOS, we allow ANY
		// process to call sys_wait(P).)
		...
		pid_t p = current->p_registers.reg_eax;
		if (p <= 0 || p >= NPROCS || p == current->p_pid
  1001f7:	eb 17                	jmp    100210 <interrupt+0x155>
		    || proc_array[p].p_state == P_EMPTY)
			current->p_registers.reg_eax = -1;
		else if (proc_array[p].p_state == P_ZOMBIE)
  1001f9:	83 f9 03             	cmp    $0x3,%ecx
  1001fc:	75 0b                	jne    100209 <interrupt+0x14e>
			current->p_registers.reg_eax = proc_array[p].p_exit_status;
  1001fe:	8b 92 08 92 10 00    	mov    0x109208(%edx),%edx
  100204:	89 50 20             	mov    %edx,0x20(%eax)
  100207:	eb 07                	jmp    100210 <interrupt+0x155>
		else
			//current->p_registers.reg_eax = WAIT_TRYAGAIN;
			/*current->p_waiting = 1;*/ current->p_state = P_BLOCKED;
  100209:	c7 40 48 02 00 00 00 	movl   $0x2,0x48(%eax)
		schedule();
  100210:	e8 77 fe ff ff       	call   10008c <schedule>
  100215:	eb fe                	jmp    100215 <interrupt+0x15a>
...
start(void)
{
  ...
  10021b:	68 40 05 00 00       	push   $0x540
  100220:	6a 00                	push   $0x0
  100222:	68 bc 91 10 00       	push   $0x1091bc
  100227:	e8 40 03 00 00       	call   10056c <memset>
  10022c:	ba bc 91 10 00       	mov    $0x1091bc,%edx
  100231:	31 c0                	xor    %eax,%eax
  100233:	83 c4 10             	add    $0x10,%esp
	for (i = 0; i < NPROCS; i++) {
		proc_array[i].p_pid = i;
  100236:	89 02                	mov    %eax,(%edx)
	int whichprocess;
	pid_t i;
  ...
  100239:	c7 42 48 00 00 00 00 	movl   $0x0,0x48(%edx)
	int whichprocess;
	pid_t i;
	...
	memset(proc_array, 0, sizeof(proc_array));
	for (i = 0; i < NPROCS; i++) {
  100240:	83 c2 54             	add    $0x54,%edx
  100243:	83 f8 10             	cmp    $0x10,%eax
  100246:	75 ee                	jne    100236 <start+0x1f>
		proc_array[i].p_pid = i;
		proc_array[i].p_state = P_EMPTY;
	}
  ...
  100248:	c7 05 64 9f 10 00 10 	movl   $0x109210,0x109f64
  10024f:	92 10 00 
	...
	// first process.
	segments_init();
  100252:	e8 71 00 00 00       	call   1002c8 <segments_init>
	special_registers_init(current);
  100257:	83 ec 0c             	sub    $0xc,%esp
  10025a:	ff 35 64 9f 10 00    	pushl  0x109f64
  100260:	e8 e2 01 00 00       	call   100447 <special_registers_init>

	// Erase the console, and initialize the cursor-position shared
	// variable to point to its upper left.
	console_clear();
  100265:	e8 2d 01 00 00       	call   100397 <console_clear>

	// Figure out which program to run.
	cursorpos = console_printf(cursorpos, 0x0700, "Type '1' to run mpos-app, or '2' to run mpos-app2.");
  10026a:	83 c4 0c             	add    $0xc,%esp
  10026d:	68 fc 09 10 00       	push   $0x1009fc
  100272:	68 00 07 00 00       	push   $0x700
  100277:	ff 35 00 00 06 00    	pushl  0x60000
  10027d:	e8 4c 07 00 00       	call   1009ce <console_printf>
  100282:	83 c4 10             	add    $0x10,%esp
  100285:	a3 00 00 06 00       	mov    %eax,0x60000
	do {
		whichprocess = console_read_digit();
  10028a:	e8 4b 01 00 00       	call   1003da <console_read_digit>
	} while (whichprocess != 1 && whichprocess != 2);
  10028f:	8d 58 ff             	lea    -0x1(%eax),%ebx
  100292:	83 fb 01             	cmp    $0x1,%ebx
  100295:	77 f3                	ja     10028a <start+0x73>
	console_clear();
  100297:	e8 fb 00 00 00       	call   100397 <console_clear>
  ...
  10029c:	50                   	push   %eax
  10029d:	50                   	push   %eax
  10029e:	a1 64 9f 10 00       	mov    0x109f64,%eax
  1002a3:	83 c0 34             	add    $0x34,%eax
  1002a6:	50                   	push   %eax
  1002a7:	53                   	push   %ebx
  1002a8:	e8 cf 01 00 00       	call   10047c <program_loader>

	// Set the main process's stack pointer, ESP.
	current->p_registers.reg_esp = PROC1_STACK_ADDR + PROC_STACK_SIZE;
  1002ad:	a1 64 9f 10 00       	mov    0x109f64,%eax
  1002b2:	c7 40 40 00 00 2c 00 	movl   $0x2c0000,0x40(%eax)

	// Mark the process as runnable!
	current->p_state = P_RUNNABLE;
  1002b9:	c7 40 48 01 00 00 00 	movl   $0x1,0x48(%eax)

	// Switch to the main process using run().
	run(current);
  1002c0:	89 04 24             	mov    %eax,(%esp)
  1002c3:	e8 68 01 00 00       	call   100430 <run>
...
segments_init(void)
{
  ...
  1002c8:	b8 fc 96 10 00       	mov    $0x1096fc,%eax
...
segments_init(void)
{
  ...
  1002e9:	88 15 9c 1a 10 00    	mov    %dl,0x101a9c
	...
	for (i = 0; i < sizeof(interrupt_descriptors) / sizeof(gatedescriptor_t); i++)
		SETGATE(interrupt_descriptors[i], 0,
  1002ef:	ba 64 97 10 00       	mov    $0x109764,%edx
segments_init(void)
{
	int i;
...
segments_init(void)
{
  ...
  100312:	c7 05 00 97 10 00 00 	movl   $0x80000,0x109700
  100319:	00 08 00 
	kernel_task_descriptor.ts_ss0 = SEGSEL_KERN_DATA;
  10031c:	66 c7 05 04 97 10 00 	movw   $0x10,0x109704
  100323:	10 00 
	...
	for (i = 0; i < sizeof(interrupt_descriptors) / sizeof(gatedescriptor_t); i++)
		SETGATE(interrupt_descriptors[i], 0,
  100325:	66 89 0c c5 64 97 10 	mov    %cx,0x109764(,%eax,8)
  10032c:	00 
...
run(process_t *proc)
{
  100430:	8b 44 24 04          	mov    0x4(%esp),%eax
	current = proc;
  100434:	a3 64 9f 10 00       	mov    %eax,0x109f64
